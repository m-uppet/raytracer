...
color ray_color(const ray& r, const hittable& world, int depth) {
    ...
    // If we exceed maximum depth limit (recursive function !), return no light contribution
    if (depth <= 0) {
        return color(0, 0, 0);
    }

    if (world.hit(r, 0.001, infinity, rec)) { // Do not use 0 as lower bound to avoid shadow acne
        ray scattered;
        color attenuation;
...
int main() {
    ...
    // world.add(make_shared<sphere>(point3(0,-1000,0), 1000, ground_material));
    // auto material1 = make_shared<dielectric>(1.5);
    // world.add(make_shared<sphere>(point3(0, 1, -1), 1.0, material1));

    // auto material2 = make_shared<lambertian>(color(0.4, 0.2, 0.1));
    // world.add(make_shared<sphere>(point3(-4, 1, -1), 1.0, material2));

    // auto material3 = make_shared<metal>(color(0.7, 0.6, 0.5), 0.0);
    // world.add(make_shared<sphere>(point3(4, 1, -1), 1.0, material3));
